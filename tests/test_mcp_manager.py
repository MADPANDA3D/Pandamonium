...
def test_mcp_call_forwards_declared_name_and_arguments_without_translation():
    class FakeSession:
        def __init__(self):
            self.call = None
    ...
    arguments = {"query": "", "configuredOnly": True}

    result = asyncio.run(manager.call_tool(
        "mcp__fixture__portal.find_tools", arguments
    ))

    assert result["exit_code"] == 0
    assert session.call == ("portal.find_tools", arguments)
...
def test_portal_uses_declared_generic_connection_entrypoints():
    manager = McpManager()
    manager._connections["portal-fixture"] = {
        "status": "connected",
        "name": "Acme MCP Portal",
