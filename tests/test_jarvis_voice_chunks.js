...
assert.match(index, /class="jarvis-selector-status"[^>]*>Discovering configured choices…<\/div>/);
assert.match(source, /fetchJson\('\/api\/selector-catalog'\)/);
assert.match(source, /\['agent', 'Agents'\]/);
assert.match(source, /\['worker', 'Workers'\]/);
assert.match(source, /\['model', 'Models'\]/);
...
assert.match(serviceWorker, /CACHE_NAME = 'pandamonium-v382'/);
