...
const CURRENT_CACHE = CURRENT_WORKER.match(/const CACHE_NAME = '(pandamonium-v(\d+))';/)[1];
const FUTURE_CACHE = `pandamonium-v${Number(CURRENT_CACHE.match(/\d+$/)[0]) + 1}`;
const FUTURE_WORKER = CURRENT_WORKER.replace(CURRENT_CACHE, FUTURE_CACHE);
...
for (const [scenario, bridgeReload] of [
  ...
  expect(FUTURE_WORKER).toContain(`const CACHE_NAME = '${FUTURE_CACHE}';`);
  ...
  const sourceCache = bridgeReload ? 'pandamonium-v387' : CURRENT_CACHE;
  const targetCache = bridgeReload ? CURRENT_CACHE : FUTURE_CACHE;
