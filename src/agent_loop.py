...
- Calendar lookup and comparison use `read_calendar`, which synchronizes connected CalDAV data first.
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    "read_calendar": "- ```read_calendar``` — Synchronize and read the authenticated user's calendars without changing them. Args (JSON): {\"action\":\"list_events|list_calendars\", \"start\":\"ISO datetime\"?, \"end\":\"ISO datetime\"?, \"calendar\":\"name or id\"?}. Use this for every Calendar lookup or comparison; if freshness could not be confirmed, say so explicitly.",
