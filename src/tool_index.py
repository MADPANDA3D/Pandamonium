...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "ui_control": "Control the UI and toggle tools on/off. ORACLE: `oracle_protocol engage`, `oracle_protocol shutdown`, or `oracle_protocol style <normal|CRT|NVG|FLIR|anime|noir|snow>`. Use this when Leo asks to activate/open/close ORACLE or change its visual mode. Tool toggles: shell (bash), search (web), research, browser, documents, incognito. Open panels (documents library, gallery, email inbox, sessions, notes, memories/brain, skills, settings, cookbook) via `open_panel <name>`. Use `open_email_reply <uid> <folder> reply <body text>` (or structured body) to open an email reply draft document without sending. Also switches between chat/agent modes, changes the current model, and applies/creates themes.",
