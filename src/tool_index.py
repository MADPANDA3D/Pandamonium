...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_workspace": "Set, clear, or report the active workspace folder for this chat. Use it when the user says to work out of, switch to, or change the folder/workspace/directory the agent should use ('work out of /srv/app', 'use the Home Lab folder'), or to clear it. The folder is validated by the server (must exist, not a filesystem root or sensitive path) and persisted on the chat so every client sees it.",
    "ssh_node": "Run a bounded operation on one SAVED SSH connection (Settings > SSH Connections): list a folder, read a file, or run a command the connection's allowlist permits. It never invents a target; the connection must already be saved. Output, run time, and command syntax are bounded, every call is audited, and results cite the exact node and path. Use for 'SSH into my VPS and get/check X' when the node is configured.",
    "nextcloud_files": "Read-only access to the owner's connected Nextcloud (Settings > Integrations): list a folder, search file names, or read a text file. There is no write or upload path. Content is bounded, secret-shaped paths are excluded, and results cite the exact node and path. Use for 'my Nextcloud files', 'the file on Nextcloud', or reading a document stored on the connected cloud instance.",
    "android_device": "Governed Android emulator/ADB control for the configured SDK. Discover the SDK and connected devices/emulators (serial, model, Android/API version, boot state, authorization), list installed AVDs, start/stop/reboot/wait for an AVD, install an APK, launch or force-stop a package, open a deep link, capture a screenshot or screen recording, read bounded logcat, and send explicit tap/swipe/text/key input. Device actions need an explicit ready serial. No arbitrary adb shell, no host shell, no uninstall/wipe/clear-data, no credential or Play Store automation. Use for 'the android emulator', 'AVD', 'adb devices', 'install this apk', 'take a screenshot on the emulator', or Android app testing requests.",
